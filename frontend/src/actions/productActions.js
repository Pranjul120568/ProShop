  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(
        `https://proshop-69.herokuapp.com/api/products?keyword=${keyword}&pageNumber=${pageNumber}&pageSize=${pageSize}&filter=${filter}`
      );
...
export const listProductDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `https://proshop-69.herokuapp.com/api/products/${id}`
    );
    //after successfuling fetching the data dispatch action request success to update loading to false and product to action.payload
    dispatch({
...
export const createProduct = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(
      `https://proshop-69.herokuapp.com/api/products`,
      {},
      config
    );
...
export const updateProduct = (product) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `https://proshop-69.herokuapp.com/api/products/${product._id}`,
      product,
      config
    );
...
export const listFeaturedProducts = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `https://proshop-69.herokuapp.com/api/products/featured`
    );
