...
export const login = (email, password) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      "https://proshop-69.herokuapp.com/api/users/login",
      { email, password },
      config
    );
...
export const register = (name, email, password) => async (dispatch) => {
  try {
    ...
    //first argument is url, second is credentials, third is config
    const { data } = await axios.post(
      "https://proshop-69.herokuapp.com/api/users",
      { name, email, password },
      config
    );
    //send off to reducer to update state
...
export const getUserDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(
      `https://proshop-69.herokuapp.com/api/users/${id}`,
      config
    );
    //send off to reducer to update state
    dispatch({
...
export const updateUserProfile = (user) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `https://proshop-69.herokuapp.com/api/users/profile`,
      user,
      config
    );
    //send off to reducer to update state
...
export const updateUser = (user) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `https://proshop-69.herokuapp.com/api/users/${user._id}`,
      user,
      config
    );
    //send off to reducer to update state
